...
def get_files(file_dir):
	...
	for file in os.listdir(file_dir):
		...
		if 'cat' in name[0]:
			...
			dags.append(file_dir + file)
			label_dags.append(1)
		elif 'apple' in name[0]:
			apples.append(file_dir + file)
			label_apples.append(2)
		else:
			if 'dog' in name[0]:
				dogs.append(file_dir + file)
				label_dogs.append(3)
		image_list = np.hstack((cats,dogs,dags,apples))
		label_list = np.hstack((label_cats,label_dogs,label_dags,label_apples))
...
def read_img(path):
    ...
    imagelist = np.hstack((images))
    lableslist = np.hstack((imagelable))
    # 把标签和图片都放倒一个 temp 中 然后打乱顺序，然后取出来
    temp = np.array([imagelist, lableslist])
    temp = temp.transpose()
    # 打乱顺序
    np.random.shuffle(temp)
